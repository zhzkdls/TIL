...
//엄첨나게 많은 숫자를 배열을 했을때 일일이 숫자를 지정해서 해주는게 아니라 숫자들.length 라는 함수를 이용 length는 수의 길이만큼 자동으로 인식해주는 함수이다
...
// 반복문을 사용해 수.length함수를 사용해서 수의 길이만큼 인식을 해준다

// 가장 큰수에서 배웠던것을 응용하면 됩니다 이번에는 더하기를 하고 그 더한 숫자를 누적에 저장이 됩니다 그리고 누적이라는 변수에 저장된 값이 업데이트가 됩니다 이런씩을 반복을 하면 모든 수를 합한 숫자가 나옵니다.
//숫자를 0부터 시작해서 배열에 순서를 하나씩 하나씩 더해가는 방식이다.

let 수 = [2, 4, 6, 7, 9, 20, 30, 40, 50, 60, 70, 80, 100, 101, 101, 2, 4, 6, 7, 9, 20, 101]
